    ...
    for transaction in transactions:
        ...
        if (transaction.billing_date-date.today()).days < 0:
            ...
            if (billing.time_stamp - date.today()).days < 7:
                ...
                try:
                    owner_notif = OwnerNotification.objects.get(owner_id=transaction.room_id.catalog_id.property_id.owner_id, title=title, time_stamp=date.today())
                    print('owner notif exists')
                except OwnerNotification.DoesNotExist:
                    owner_notif = OwnerNotification(owner_id=transaction.room_id.catalog_id.property_id.owner_id, title=title, body=body, time_stamp=date.today())
                    owner_notif.save()
